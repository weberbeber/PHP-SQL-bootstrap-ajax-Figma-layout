...
/**
 * Найти ближайших сотрудников к заданному району
 * 
 * @param string $area
 * 
 * @return string array
 * 
 */
function searchNearbyWorkers($area) 
{
	global $areas, $nearby, $workers;

	// Итоговый массив логинов
	$foundWorkers = [];

	// Ближайшие районы
	$nearbyAreasNumbers = $nearby[$areaNumber = array_search($area, $areas)];

	array_unshift($nearbyAreasNumbers, $areaNumber);

	// Поиск сотрудников из ближайших районов
	foreach ($nearbyAreasNumbers as $value)
	{
		foreach ($workers as $key => $worker)
		{
			if (strcmp($workers[$key]['area_name'], $areas[$value]) == 0)
				array_push($foundWorkers, $workers[$key]['login']);
		}			
	}
...
echo '<pre>';
print_r(searchNearbyWorkers('Центр'));
echo '</pre>';
